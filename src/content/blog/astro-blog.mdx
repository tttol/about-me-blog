...
Astroとはフロントエンド向けのJavaScriptベースのライブラリです。Reactほどのリッチな機能は必要ないが、Vanilla JSで全部書くには大変…というどっちつかずな悩みを解決してくれるのがAstroです。今回の私のように、単純なブログを作成したい場合にぴったりなライブラリです。
...
改行位置がちょっと気になるが、まあ許容してます。[budoux](https://github.com/google/budoux)というライブラリを使うと改行の位置をいい感じにできるようですが、現状は手を出してません。そのうちやるかも。

### CI/CD
記事を執筆したら自動でデプロイが走るようにCI/CDを組んでいます。

仕組みは単純で、`npm run build`で生成したdistディレクトリをS3にアップロードするという2ステップです。
